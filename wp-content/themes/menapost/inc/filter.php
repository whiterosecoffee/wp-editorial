...
function mp_get_articles_by_post_tag( $post_tag, $filter, $limit, &$count, $offset ) {
	...
	if( is_array( $post_tag ) ) {
		if( is_numeric( $post_tag[0] ) )
			...
			$where = array_map( 'Utilities::add_quotes_and_escape', $post_tag );
			$where = implode( ',', $where );
			$count = count( $post_tag );
			$query = "SELECT object_id FROM wp_term_relationships JOIN wp_term_taxonomy ON wp_term_taxonomy.term_taxonomy_id = wp_term_relationships.term_taxonomy_id JOIN wp_terms ON wp_terms.term_id = wp_term_taxonomy.term_id AND wp_terms.slug IN ( $where ) JOIN wp_posts ON wp_posts.ID = object_id AND wp_posts.post_status = 'publish' AND wp_posts.post_type = 'post' group by object_id having count(1) = $count";
		}
	} else {
