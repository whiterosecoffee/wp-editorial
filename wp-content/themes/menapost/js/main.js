...
(function ($) {
	...
	$(document).ready(function() {		
		...
		if( $('[data-page="article-detail"]').length ) {
			...
			verticalActivityBar.attr( 'data-min-width', 768 );
			verticalActivityBar.attr( 'data-bottom-offset', $('.activitybar-vertical ul').css('marginBottom').replace('px', '') );
			sidebar.attr( 'data-min-width', 992 );

			$(window).resize(function() {
				if( viewport().width >= 1200 )
					sidebar.attr( 'data-offset-left', $('body').offset().left + 'px' );
				else {
					sidebar.attr( 'data-offset-left', '0' );
					sidebar.css( 'left', 0 );
				}
			});
			sidebar.attr( 'data-offset-left', sidebar.offset().left + 'px' );
...
(function($) {
	...
	if( $('.article-detail').size() !== 0 ) {
		...
		function trackMouseMove(e) {
			...
			if(e.pageY <= topexit && currentMousePosition < lastMousePosition) {
