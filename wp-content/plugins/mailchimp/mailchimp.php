<?php
/*
Plugin Name: MailChimp DO NOT UPDATE
Description: MailChimp.php has been edited to allow for single-optin newsletter sign up. Arabic error messages have also been added. The MailChimp plugin allows you to quickly and easily add a signup form for your MailChimp list.
Version: 1.4.2
...
function mailchimpSF_signup_submit() {
		...
		if (is_array($opt_val) && isset($opt_val['area'])) {
			...
			if ($var['req'] == 'Y') {
				if (empty($opt_val['addr1']) || empty($opt_val['city'])) {
					$errs[] = sprintf(__("الرجاء ادخال بياناتك %s.", 'mailchimp_i18n'), esc_html($var['name']));//You Must Fill In
					$success = false;
				}
			}
		...
		if ($success) {
			...
			if (!$retval) {
				switch($api->errorCode) {
					case '105' :
						$errs[] = __("الرجاء المحاولة لاحقاً", 'mailchimp_i18n').'.'; // Please try again later
						break;
					case '214' :
						$msg = __("لقد قمت بالتسجيل سابقاًًً", 'mailchimp_i18n') . '.'; // That email address is already subscribed to the list
						...
						if (!$api->errorCode) {
							...
							$msg .= ' ' . sprintf(__('<a href="%s">انقر هنا لتحديث بياناتك الشخصية</a>', 'mailchimp_i18n'), "http://$username.$dc.list-manage.com/subscribe/send-email?u=$uid&id=$listId&e=$eid");//Click here to update your profile.
						}
					...
					case '250' :
						...
						$errs[] = sprintf(__("الرجاء ادخال بياناتك %s.", 'mailchimp_i18n'), esc_html($mv_tag_keys[$field]['name'])); // You Must Fill In
						break;
					case '254' :
					...
					case '270' :
						...
						break;
					case '502' :
						$errs[] = __("عنوان البريد الإلكتروني غير صحيح. يرجى استخدام عنوان بريد إلكتروني صحيح", 'mailchimp_i18n').'.';
						break;
					default:
	...
	if (count($errs) > 0) {
		...
		$msg = "<strong class='mc_success_msg'>".esc_html(__("شكرا على الاشتراك في نشرة كسرة", 'mailchimp_i18n'))."</strong>";
	}
